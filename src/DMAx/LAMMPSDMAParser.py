...
class TwoptDataParser:
    def __init__(self, calc_dir):
        self.calc_dir = calc_dir
    
    def gibbsparser(self, plot=False, plot_title="Gibbs Free Energy vs runtime", **kwargs):
        simulation_dir = kwargs.get("simulation_dir", self.calc_dir)
        os.chdir(simulation_dir)
        eng_file = [i for i in os.listdir(simulation_dir) if i[-3:] == "eng"][0]
        with open(eng_file, 'r') as file:
            lines = file.readlines()     
        time = [float(i.split()[2]) for i in lines if "------------ Step" in i]
        time = np.array([i - time[0] for i in time])
        tot_engs = np.array([float(i.split()[2]) for i in lines if "TotEng   =" in i]) * 4184 #J/mol
        temperatures = np.array([float(i.split()[8]) for i in lines if "TotEng   =" in i]) #K
        pressures = np.array([float(i.split()[2]) for i in lines if "Press    =" in i]) * 101325 #Pa
        volumes = np.array([float(i.split()[5]) for i in lines if "Press    =" in i]) * 1e-30 #m^3
        thermo_file = [i for i in os.listdir(simulation_dir) if i[-6:] == "thermo"][0]
        with open(thermo_file, 'r') as file:
            lines = file.readlines()
        entropy = np.array([float(i.split()[8]) for i in lines if "Sq_(J/mol_K/SimBox)" in i])
        gibbs = tot_engs - temperatures * entropy + pressures #* volumes
        gibbs = np.array([float(i.split()[8]) for i in lines if "Aq__(kJ/mol/SimBox)" in i])
        avg_gibbs = np.average(gibbs)
        if plot:
            plt.title(plot_title)
            plt.xlabel("Time (fs)")
            plt.ylabel("Gibbs Free Energy")
            plt.scatter(time, gibbs, s=2)
        return avg_gibbs
    
    def selfdiffparser(self, **kwargs):
        simulation_dir = kwargs.get("simulation_dir", self.calc_dir)
        thermo_file = [i for i in os.listdir(simulation_dir) if i[-6:] == "thermo"][0]
        with open(thermo_file, 'r') as file:
            lines = file.readlines()
        self_diff = np.array([float(i.split()[8]) for i in lines if "Diffus(cm2/s_,_1/s)" in i])
        return self_diff
    
class TwoptGibbsParser(TwoptDataParser):
    def phase_difference_parser(self, calc_dirs_dict, plot=True):
        # Calc_dirs_dict should be like: {"Amorphous": "/home/ricardosb/...", "Crystalline": "/home/ricardosb/..."} or {"Solid": "/home/ricardosb/...", "Liquid": "/home/ricardosb/..."}
        gibbs_dict = {}
        for phase, directory in zip(calc_dirs_dict.keys(), calc_dirs_dict.values()):
            os.chdir(directory)
            gibbs_dict[phase] = {"temperatures": [], "gibbs": []}
            os.chdir("temperatures")
            for temp_dir in os.listdir("."):
                os.chdir(temp_dir)
                gibbs_dict[phase]["temperatures"].append(float(temp_dir[:-1]))
                gibbs_dict[phase]["gibbs"].append(self.gibbsparser(**{"simulation_dir": os.getcwd()}))
                os.chdir("../")
            sorted_data = sorted(zip(gibbs_dict[phase]["temperatures"], gibbs_dict[phase]["gibbs"]))
            gibbs_dict[phase]["temperatures"], gibbs_dict[phase]["gibbs"] = map(list, zip(*sorted_data))
        if plot:
            plt.title("Gibbs Free Energy vs Temperature at Phase Transition")
            plt.xlabel("Temperature (K)")
            plt.ylabel("Gibbs Free Energy (J/mol)")
            for phase in gibbs_dict.keys():
                plt.plot(gibbs_dict[phase]["temperatures"], gibbs_dict[phase]["gibbs"], marker="o", label=phase)
            plt.legend()
        return gibbs_dict
    
class TwoptSelfDiffParser(TwoptDataParser):
    def self_diffusion_evolution_parser(self, calc_dir, plot=True):
        os.chdir(calc_dir)
        os.chdir("temperatures")
        temperature_list = []
        self_diffusion_list = []
        for temp_dir in os.listdir("."):
            os.chdir(temp_dir)
            temperature_list.append(float(temp_dir[:-1]))

            self_diffusion_list.append(self.selfdiffparser(**{"simulation_dir": os.getcwd()}))
            os.chdir("../")
        sorted_data = sorted(zip(temperature_list, self_diffusion_list))
        temperature_list, self_diffusion_list = map(list, zip(*sorted_data))
        if plot:
            plt.title("Self-Diffusion Constant vs Temperature")
            plt.xlabel("Temperature (K)")
            plt.ylabel("Self-Diffusion Constant (cm^2/s)")
            plt.plot(temperature_list, self_diffusion_list, marker="o")
        return dict(zip(temperature_list, self_diffusion_list))
